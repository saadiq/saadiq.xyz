...
Ghost decides a member's source from `sessionStorage['ghost-history']`, written by `member-attribution.min.js` and posted as `urlHistory` on `send-magic-link`. Two things about this repo's setup are non-obvious and were each derived the hard way (2026-08-26):
